...
const Photo = ({ src, alt, caption, style, lift = false, className = "", children }) => {
  return (
    <div
    ...
    >
      <div className="photo__img" style={{ backgroundImage: `url(${src})` }} />
...
const TopoLine = ({ style }) => (
  <svg width="100%" height="40" viewBox="0 0 800 40" preserveAspectRatio="none" style={style}>
    <path
...
const CompassMark = ({ size = 18 }) => (
  <svg width={size} height={size} viewBox="0 0 18 18" fill="none">
    <circle cx="9" cy="9" r="7.5" stroke="currentColor" strokeWidth="1" opacity="0.6" />
...
const Reveal = ({ children, delay = 0, as: As = "div", style, ...rest }) => {
  ...
  return (
    <As ref={ref} className={`reveal ${shown ? "in" : ""}${rest.className ? " " + rest.className : ""}`} style={style}>
      {children}
    </As>
  );
};
...
const Icon = ({ name, size = 18, color = "currentColor" }) => {
  ...
  switch (name) {
    case "phone":
      return (<svg {...props}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.86 19.86 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.86 19.86 0 0 1 2.12 4.18 2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.37 1.9.72 2.81a2 2 0 0 1-.45 2.11L8.09 10a16 16 0 0 0 6 6l1.36-1.3a2 2 0 0 1 2.11-.45c.91.35 1.85.59 2.81.72A2 2 0 0 1 22 16.92Z"/></svg>);
    case "mail":
      return (<svg {...props}><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg>);
    case "wa":
      return (<svg {...props}><path d="M21 11.5a8.4 8.4 0 0 1-1.2 4.3L21 21l-5.3-1.4a8.5 8.5 0 1 1 5.3-8.1Z"/><path d="M8 9.5c0 4 3 7 7 7"/></svg>);
    case "ig":
      return (<svg {...props}><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="0.6" fill={color}/></svg>);
    case "arrow":
    ...
    case "elevation":
      return (<svg {...props}><path d="M3 18l5-7 4 5 3-4 6 8H3z"/></svg>);
    case "calendar":
      return (<svg {...props}><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M8 3v4M16 3v4M3 10h18"/></svg>);
    case "users":
      return (<svg {...props}><circle cx="9" cy="8" r="3.5"/><path d="M2.5 20a6.5 6.5 0 0 1 13 0"/><circle cx="17" cy="9" r="2.5"/><path d="M22 19a4.5 4.5 0 0 0-6-4.2"/></svg>);
    case "compass":
      return (<svg {...props}><circle cx="12" cy="12" r="9"/><path d="M14 10l-1 4-4 1 1-4 4-1z" fill={color}/></svg>);
    case "leaf":
      return (<svg {...props}><path d="M5 21c8 0 14-6 14-14V4h-3C8 4 4 9 4 16c0 2 .5 3.5 1 5z"/><path d="M5 21c5-7 9-10 14-13"/></svg>);
    case "shield":
...
Object.assign(window, {
  FK_ImageSlot, Photo, PHOTOS, HeroMountains, OmPeakIcon, TopoLine, CompassMark, Reveal, Icon,
});
